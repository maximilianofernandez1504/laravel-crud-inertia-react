    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      <div>
        <Link href={route('products.edit', product.id)}>
          <Button className='mb-4'>Editar Producto</Button>
        </Link>
        {product.state === 0 ? (
          <Link href={route('products.restore', product.id)} method="patch">
            <Button className='mb-4'>Reactivar Producto</Button>
          </Link>
        ) : (
          <Link href={route('products.destroy', product.id)} method="delete" as="button">
            <Button className='mb-4'>Desactivar Producto</Button>
          </Link>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {product.images.length > 0 ? (
            ...
            <img
              src="/storage/app/public/products/default.png"
              alt="No hay imagen"
